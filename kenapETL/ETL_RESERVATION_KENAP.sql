...
JOIN KENAP.dbo.Dim_Car AS DC ON DC.License_Plate = CO.Licence_plate
JOIN KENAP.dbo.Dim_Date AS DF ON DF._Date = R.Date_From
JOIN KENAP.dbo.Dim_Date AS DT ON DT._Date = R.Date_To
JOIN KENAP.dbo.Dim_Customer AS Customer ON Customer.PESEL = R.Customer_Pesel AND R.Date_From BETWEEN Customer.Activation_Date AND ISNULL(Customer.Disactivation_Date, CURRENT_TIMESTAMP)) AS x
GO
